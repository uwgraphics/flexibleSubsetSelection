...
def errorMarkers(
    ...
    for i in range(len(x)):
        if vals1 is not None and len(vals1[i]) > 0:
            for val in vals1[i]:
                ax.plot(x[i], val, color=color1, markersize=4, marker=marker1, zorder=4)
...
def errorLines(
    ...
    lines = LineCollection(lines, colors=color, linewidths=weights.flatten(), alpha=0.2)
...
def parallelCoordinates(
    ...
    **parameters,
    ...
    if dataset is not None:
        df_dataset = pd.DataFrame(getattr(dataset, transform), columns=dataset.features)
    ...
    if subset is not None:
        df_subset = pd.DataFrame(
            getattr(subset, transform), columns=subset.dataset.features
        )
    ...
    **parameters,
    ...
    if dataset is not None:
        ...
        for i, feature in enumerate(dataset.features):
            ...
            datasetHist = np.histogram(
                getattr(dataset, transform)[feature], bins=numBins
            )
    ...
    if subset is not None:
        ...
        for i, feature in enumerate(features):
            ...
            subsetHist = np.histogram(getattr(subset, transform)[feature], bins=numBins)
