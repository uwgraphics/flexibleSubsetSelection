...
def randomData(randType, size, interval, seed) -> pd.DataFrame:
    """
    Generate random data based on the specified random generation method.

    Args:
        randType (str or list): The method or methods for random data generation.
            Supported methods: "uniform", "binary", "categorical", "normal", 
            "multimodal", "skew", and "blobs".
        size (tuple): The size of the dataset in row-major order for random 
            dataset generation. For example, (num_rows, num_columns).
        interval (tuple): The interval specifying the range of random values.
...
def uniform(size, interval, seed=None) -> pd.DataFrame:
    """
    Generate random data from a uniform distribution using numpy.

    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple): Range of values for the uniform distribution.
        seed (int, rng, optional): Seed or rng for random number generation.
...
def binary(size, interval=None, seed=None) -> pd.DataFrame:
    """
    ...
    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple): Not necessary fo binary data. Included for uniformity.
        seed (int, rng, optional): Seed or rng for random number generation.
...
def categorical(size, interval, seed=None) -> pd.DataFrame:
    """
    ...
    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple): Range of possible number of categories.
        seed (int, rng, optional): Seed or rng for random number generation.
...
def normal(size, interval, seed=None) -> pd.DataFrame:
    """
    ...
    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple): Range of values for the mean and standard deviation.
        seed (int, rng, optional): Seed or rng for random number generation.
...
def multimodal(size, interval, sigmaInterval=(0.1, 3), 
    ...
    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple): Range of possible number of modes for each feature.
        sigmaInterval (tuple, optional): Range of possible standard deviations.
        seed (int, rng, optional): Seed or rng for random number generation.
...
def skew(size, interval=(-5, 5), seed=None) -> pd.DataFrame:
    """
    ...
    Args:
        size (tuple): Number of data points and features to generate.
        interval (tuple, optional): Range of possible skewness parameters.
        seed (int, rng, optional): Seed or rng for random number generation.
    ...
    for i in range(size[1]):
        ...
        data[i, :] = scipy.stats.skewnorm.rvs(a, size=size[0], 
                                              random_state=seed)

    return pd.DataFrame(data.T)

def blobs(size, interval, numClusters=6, sigmaInterval=(0.1, 3), 
                  seed=None) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        interval (tuple): Range of values for the cluster centers.
        sigmaInterval (tuple, optional): Range of possible standard deviations.
        seed (int, rng, optional): Seed or rng for random number generation.
    ...
    data = make_blobs(n_samples=size[0], n_features=size[1], 
                                       centers=numClusters, cluster_std=sigma, 
                                       center_box=interval)
